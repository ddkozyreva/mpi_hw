...
_При запуске программы в заданиях 2 и 3 меняла параметры --ntasks-per-node на 1-5 и -N на 2 с целью исследовать производительность программы._
...
| --- | --- | --- |
| 1 процесс | 2.39 | 2.44 |
| 2 процесса | 1.33 | 1.51 |
| 3 процесса | 0.88 | 0.89 |
| 4 процесса | 0.62 | 0.63 |
| 5 процессов | 0.56 | 0.52 |

Сравнение производилось на матрице 1000х1000. Ответ во всех экспериментах один: -44.658. По времени оба решения примерно одинаково эффективны. На кластере результаты сохранены в папке output/.
___

### Задание №3

Зависимость эффективности параллелизации от числа процессов на одном узле была получена выше. Приведем таблицу на 2 узлах:

|  | Параллелизация 1 | Параллелизация 2 |
| --- | --- | --- |
| 1 процесс | 1.27 | 1.25 |
| 2 процесса | 0.65 | 0.69 |
| 3 процесса | 0.51 | 0.55 |
| 4 процесса | 0.38 | 3.61 |
| 5 процессов | 0.35 | 7.57 |

Видим, что с ростом числа процессов в основном наблюдается ускорение программы, однако во втором варианте при 4 и 5 процессах время значительно увеличилось. Также отмечу, что при 2 узлах скорость выполнения программ примерно в 2 раза выше.